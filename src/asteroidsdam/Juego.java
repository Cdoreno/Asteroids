...
/**
 * The {@code Juego} class is responsible for initializing and running the game.
...
public class Juego extends JFrame {
	...
	/**
	 * The value for {@code deathCooldown} that the Jugador respawns.
	 */
	...
	/**
	 * The Espacio instance.
	 */
	private Espacio world;
	
	/**
	 * The Reloj instance for handling the game updates.
	 */
	private Reloj logicTimer;
	...
	/**
	 * The list of Entidad objects that exist in the game world.
	 */
	private List<Entidad> entities;
	
	/**
	 * The list of Entidad objects that need to be added to the game world.
	 */
	private List<Entidad> pendingEntities;
		
	/**
	 * The Jugador instance.
	 */
	private Jugador player;
	
	/**
	 * <p>The death cooldown timer is responsible for spreading a Jugador's death
 out over time, so the player does not instantly spawn.</p>
	 ...
	 * 
	 * <p>Once the value reaches {@code INVULN_COOLDOWN_LIMIT}, the Jugador's ship
 will be vulnerable to collisions and the Jugador will regain the ability to
 shoot.</p>
	 */
	...
	/**
	 * The number of lives the Jugador has left.
	 */
	private int lives;
	...
	private Juego() {
		...
		//Create and add the Espacio instance to the window.
	...
	private void startGame() {
		...
		this.entities = new LinkedList<Entidad>();
		...
		this.logicTimer = new Reloj(FRAMES_PER_SECOND);
		while(true) {
	...
	private void updateGame() {
		...
		if(!isGameOver && areEnemiesDead()) {
			...
			for(int i = 0; i < level + 2; i++) {
				registerEntity(new Meteroito(random));
			}
		}
		...
		if(showLevelCooldown == 0) {
			...
			for(int i = 0; i < entities.size(); i++) {
				Entidad a = entities.get(i);
				for(int j = i + 1; j < entities.size(); j++) {
					Entidad b = entities.get(j);
			...
			Iterator<Entidad> iter = entities.iterator();
			while(iter.hasNext()) {
