...
public class Jugador extends Entidad {
	 ...
	 */
	private List<Disparo> bullets;
	
	/**
	 * Initializes a new Player instance.
	 */
	public Jugador() {
		super(new Vector(Espacio.WORLD_SIZE / 2.0, Espacio.WORLD_SIZE / 2.0), new Vector(0.0, 0.0), 10.0, 0);
	...
	public void reset() {
		this.rotation = DEFAULT_ROTATION;
		position.set(Espacio.WORLD_SIZE / 2.0, Espacio.WORLD_SIZE / 2.0);
	...
	public void update(Juego game) {
		 ...
		 */
		Iterator<Disparo> iter = bullets.iterator();
		while(iter.hasNext()) {
		...
		if(firingEnabled && firePressed && fireCooldown <= 0 && overheatCooldown <= 0) {
			...
			if(bullets.size() < MAX_BULLETS) {
				...
				Disparo bullet = new Disparo(this, rotation);
				bullets.add(bullet);
	...
	public void handleCollision(Juego game, Entidad other) {
		//Kill the player if it collides with an Meteroito.
	...
	public void draw(Graphics2D g, Juego game) {
		...
		if(!game.isPlayerInvulnerable() || game.isPaused() || animationFrame % 20 < 10) {
			 ...
			 * transformations will be handled by the Espacio before calling the draw
			 * function.
			 */
